...
// 恢复会重新构造执行器；默认调用 ID 必须只由 Run 的逻辑调用位置决定。
test('Single Tool 默认调用 ID 在同一 Run 重建后应保持稳定', async () => {
  const invoker = new FakeToolInvoker();
  const executor = new SingleToolRuntimeExecutor(
    new FakeSingleToolSelector(),
    invoker,
    new FakeSingleToolResponseProvider(),
    createExecutorOptions('stable-single'),
  );
  const request = createRequest(RouteKind.SINGLE_TOOL);

  await collectEvents(executor.execute(request));
  await collectEvents(executor.execute(request));
  await collectEvents(executor.execute({
    ...request,
    run: { ...request.run, id: 'run-single-tool-other' },
  }));

  assert.equal(invoker.calls[0].toolCallId, invoker.calls[1].toolCallId);
  assert.equal(invoker.calls[0].idempotencyKey, invoker.calls[1].idempotencyKey);
  assert.notEqual(invoker.calls[0].toolCallId, invoker.calls[2].toolCallId);
});
