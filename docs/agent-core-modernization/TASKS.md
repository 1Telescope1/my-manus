...
4. Acceptance 全部满足后，在任务目录填写实施日志和验证证据，再将状态改为 `done`。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| RUNTIME-101 | `ready` | — | 建立可持久化的运行语义 | 定义 AgentRun、RunStep、ToolCallRecord、Checkpoint、Interruption 和状态转换 | 类型、状态转换测试和仓储接口评审通过；非法转换被拒绝 | — | 2026-07-16：初始化 |
| RUNTIME-102 | `proposed` | RUNTIME-101 | 将运行状态从 Session JSON 中分离 | 增加 Prisma 模型、迁移和仓储实现，使用乐观版本控制 | 可创建、查询、更新 Run；并发更新不会静默覆盖；迁移可回滚 | — | 2026-07-16：初始化 |
...
| RUNTIME-108 | `proposed` | RUNTIME-103, RUNTIME-105, COMPAT-101 | 将新内核接入现有会话服务 | AgentTaskRunner 通过运行模式选择 legacy 或 v2 | legacy/v2 均可完成现有聊天流程；切换不破坏历史 Session | — | 2026-07-16：初始化 |
...
| ID | Status | Dependencies | Intent | Design | Acceptance | Evidence | Last Updated |
| --- | --- | --- | --- | --- | --- | --- | --- |
| TOOL-101 | `ready` | — | 统一描述内置、MCP 和 Agent 工具 | 建立 ToolDescriptor、capability、risk 和 registry 接口 | 三类工具可注册、查询和检测名称冲突；领域层无厂商类型 | — | 2026-07-16：初始化 |
...
| ID | Status | Dependencies | Intent | Design | Acceptance | Evidence | Last Updated |
| --- | --- | --- | --- | --- | --- | --- | --- |
| COMPAT-101 | `ready` | — | 让新 Runtime 继续服务现有 UI | 定义 Runtime Event 到旧 Event 的 Adapter，新增字段均可选 | 现有 UI 不修改即可消费 v2 模拟事件；sequence 可去重 | — | 2026-07-16：初始化 |
...
| EVAL-102 | `ready` | — | 固化现有 API/Event 行为 | 为 Session、SSE、Plan、Step、Tool、Wait、Done 建合同测试 | 当前 legacy 行为有基线；事件顺序和必填字段被验证 | — | 2026-07-16：初始化 |
