...
| 命令或场景 | 结果 | 备注 |
| --- | --- | --- |
| `npm run test:contract`（`api/`） | Pass | 34 tests passed，0 failed；新增 7 项 RUNTIME-102 合同测试并保留全部既有回归。 |
| `npm run test:contract:typecheck`（`api/`） | Pass | 产品代码、合同测试和集成测试 TypeScript 检查成功。 |
...
| candidate.version 与 expectedVersion 不同 | 不写数据库并返回版本冲突 | Pass，仓储分支与合同测试覆盖。 |
| 非法 Run 状态变化 | 不写数据库并返回 `invalid_status_transition` | Pass，复用 RUNTIME-101 状态机。 |
| 相同幂等键、相同请求指纹 | 返回 `existing`，不创建第二条记录 | Pass，合同和真实数据库均验证。 |
| 相同幂等键、不同请求指纹 | 返回 `key_conflict` | Pass，合同和真实数据库均验证。 |
| Checkpoint 重复、内容冲突、跳号或事件水位回退 | 分别返回可区分结果 | Pass，专项合同测试覆盖所有分支。 |
...
| legacy Session/SSE | 行为不变 | Pass，既有合同回归全部通过；本任务没有接入执行器。 |

## Completion Evidence

- 相关提交或 PR：未创建。
- 评测或运行报告：合同测试 34/34、真实 PostgreSQL 集成测试 1/1 通过。
