...
本任务建立一层厂商无关的执行边界：路径负责最小编排，具体模型、工具、Workflow 和 Planned Agent 由注入端口实现；统一基类负责输入保护、事件 envelope、等待、成功和失败语义。
...
- 通过厂商无关的注入端口隔离模型、工具、Workflow 和 Planned Agent，领域执行器不依赖厂商 SDK 或 legacy 类型。
