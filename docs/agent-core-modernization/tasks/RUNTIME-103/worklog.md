...
- 将七个分支重复传递的 Checkpoint、工具分类、中断集合和当前 Run 收敛为一次共享现场。
- 增加局部计划工厂，分支只保留 `disposition` 与 `reason`。
- 保留未知副作用、审批、用户输入、Run 状态、缺少快照和普通恢复的原有优先级。
- RUNTIME 恢复专项和全量契约 159/159、测试类型检查与生产构建通过。
